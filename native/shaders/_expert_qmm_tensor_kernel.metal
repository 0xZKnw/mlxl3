...
if (local_row_begin >= segment_rows) return;
uint row_begin = segment_begin + local_row_begin;
uint output_tile = WEIGHT_TILE_OFFSET + expert * TILES_PER_EXPERT
    + column_block * (BN / 16u);
...
for (ushort index = 0; index < accumulator.get_capacity(); ++index) {
    ...
    if (local_row_begin + local_row >= segment_rows) continue;
