...
@pytest.mark.parametrize("rows", (2, 4, 7, 9))
...
def test_qmm_supports_vectorized_code_widths(k: int) -> None:
    rng = np.random.default_rng(46000 + k)
    encoded = rng.integers(0, 1 << k, size=(8, 8, 256), dtype=np.uint16)
    trellis = mx.array(pack_trellis(encoded, k))
    suh = mx.array(rng.uniform(0.7, 1.3, size=128).astype(np.float16))
    svh = mx.array(rng.uniform(0.7, 1.3, size=128).astype(np.float16))
    x = mx.array(rng.normal(size=(5, 128)).astype(np.float16))

    expected = mx.stack(
        [qmv_exl3(x[row], trellis, suh, svh, k, CodebookMode.DEFAULT) for row in range(5)]
    )
